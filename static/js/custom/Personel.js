...
$("body").on("click", ".ADDPersonel", function (e) {
  e.preventDefault();
  $("#NewPersonelModal").modal("show");
});
$(".NewPersonelButt").click(async function (e) {
  e.preventDefault();
  let f = $(".NewPersonelForm");
  let form = new FormData(f[0]);
  let res = await fetch("/admins/new-personel-register/", {
    method: "post",
    body: form,
  });
...
GetPersonelData = async function () {
  let res = await fetch("/admins/get-personel-data/");
  let info = await res.json();
  console.log(info);
  return info;
};
